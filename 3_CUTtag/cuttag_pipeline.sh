...
###############################################################################
# CUT&Tag pipeline (3 patient samples, single replicate R1)
#
# Steps per sample:
...
#
# Expected rawdata file pattern (paired-end, one replicate per sample):
#   rawdata/<SAMPLE>_1.fq.gz
#   rawdata/<SAMPLE>_2.fq.gz
#
# Example:
#   rawdata/PT1_CUTtag_H3K27ac_R1_1.fq.gz
#   rawdata/PT1_CUTtag_H3K27ac_R1_2.fq.gz
#   -> SAMPLE = PT1_CUTtag_H3K27ac_R1
#
# Outputs (ATAC-like structure):
#   1_cleandata/   : cleaned fastq (fastp)
#   2_bam/         : aligned + dedup BAM
...
# Example: rawdata/PT1_CUTtag_H3K27ac_R1_1.fq.gz -> PT1_CUTtag_H3K27ac_R1
...
echo "Detected CUT&Tag samples (R1 only):"
printf '  %s\n' "${SAMPLES[@]}"

echo "Pipeline for CUT&Tag (single replicate R1)"
echo "Author: Zheng Enrun (tidied)"
...
for sample in "${SAMPLES[@]}"; do
  ...
  ##########################################################################
  # 5) BigWig generation (bamCoverage)
  ##########################################################################
  cd "${ROOT_DIR}/3_bw"
